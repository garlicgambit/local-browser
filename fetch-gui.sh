...
# SUPPORT PROJECT:
# Bitcoin:
# 1Ky4PDV8ustDK89jFxYAQc41AXjp1ZwBVm
#
# Monero:
...
# GUI browsers which are supported
# - iceweasel (firefox)
# - chromium (chrome)
# - xombrero
# - arora
...
# - uzbl
#
# GUI browsers which need extra tweaks
# - dillo (needs extra firewall rules for localhost)
# - opera (requires extra repository)
...
sudo -n -u "${browser_user}" XAUTHORITY=/home/"${browser_user}"/.Xauthority "${browser}" "${page_on_disk}" &>/dev/null &
