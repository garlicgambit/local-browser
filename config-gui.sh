...
apt-get install -y sudo curl gzip "${browser}"
...
# Output example firewall rules for standard system
echo ""
echo "Below are example firewall rules to add to a standard system."
...
echo "..."
echo "iptables -A OUTPUT -p tcp --syn --dport 80 -m state --state NEW -m owner --uid-owner ${fetch_user} -j ACCEPT"
echo "iptables -A OUTPUT -p tcp --syn --dport 443 -m state --state NEW -m owner --uid-owner ${fetch_user} -j ACCEPT"
